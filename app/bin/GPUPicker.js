...
/*  	var FaceIDMaterial = function() {
	...
	let FaceIDMaterial = new THREE.ShaderMaterial ({
			uniforms: {
				baseId: {
					type: "f",
					value: 0
	...
	FaceIDMaterial.constructor = FaceIDMaterial;
	FaceIDMaterial.setBaseID = function(baseId) {
		
		this.uniforms.baseId.value = baseId;
	};
	FaceIDMaterial.setPointSize = function(size) {
		this.uniforms.size.value = size;
	};
	FaceIDMaterial.setPointScale = function(scale) {
	...
	THREE.Mesh.prototype.raycastWithID = ( function() {
		...
		return function(elID, raycaster) {
			...
			inverseMatrix.invert(this.matrixWorld); //JCT
			...
			triangle.getPlane(curPlane);
			var intersectionPoint = new THREE.Vector3(0, 0, 0);
			ray.intersectPlane(curPlane, intersectionPoint);
		...
		if (object.geometry) {
			...
			if (object.geometry.__pickingGeometry) {
				...
				if (!object.geometry.isBufferGeometry) {
					...
					__pickingGeometry = new THREE.BufferGeometry(object);
				}
				var units = 1;
				...
				const attr = new Float32Array(vertexCount);
				var ids = new THREE.BufferAttribute(attr, 1);
				//set vertex id color
			...
			object.material = FaceIDMaterial;
